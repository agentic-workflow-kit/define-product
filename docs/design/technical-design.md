...
| INV-004  | Invariant            | Output stays at product altitude: no architecture, package layout, CLI/command behavior, schema internals, or delivery sequencing. Owner: Altitude Guard.                                                                                                      | SRC-001              |
...
| SURF-002 | API and surface      | Package exports: a PRD template set plus a validator API (required-section check, AC-ID format + uniqueness + status/supersession integrity, and handoff-ready rejection of draft placeholders / AC-less PRDs). The checkable core.                            | SRC-002              |
...
| FAIL-002 | Failure              | Contract validation fails (missing section / draft placeholder content / empty Acceptance Criteria / malformed or duplicate AC ID) -> surface findings; do not silently emit a non-conformant PRD.                                                             | SRC-002              |
...
| ENF-002  | Enforcement          | Static, seeded: required-section presence check plus handoff-finish guard against draft placeholders and an empty Acceptance Criteria section. Seeded violations: a PRD missing "Acceptance Criteria" or carrying placeholder/AC-less draft content must fail. | SURF-002, SRC-002    |
...
| ENF-002 Required-section presence and handoff-finish guard enforced                       | validator unit tests; seeded fixture missing "Acceptance Criteria" or carrying placeholder/AC-less draft content must fail | `packages/prd-kit` tests via `pnpm check` |
