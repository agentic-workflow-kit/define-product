...
| AC-CITE-001  | Technical Design and Planning can cite acceptance-criteria IDs without reading implementation internals. | Active |

## Constraints

- The PRD stays at product altitude and does not prescribe architecture, packages, CLI behavior, or
  execution sequencing.
- Acceptance criteria describe recognizable product success, not delivery tasks.

## Assumptions

- Downstream artifacts can preserve product traceability if they cite stable acceptance-criteria
  IDs.
- A compact section contract is enough for M3; richer tooling can be designed later.
...
- Technical Design may cite `AC-PRD-001`, `AC-ID-001`, `AC-SCOPE-001`, and `AC-CITE-001` when
  reconciling design decisions to product intent.
- Planning may cite the same IDs when preserving product traceability into an execution plan.
